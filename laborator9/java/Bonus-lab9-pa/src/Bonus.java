	...
	static class Task {
		...
		class Node implements Comparable { 
			...
			public int compareTo(Object node) {
		        if (this.cost < ((Bonus.Task.Node)node).cost) 
		            return -1; 
		        if (this.cost > ((Bonus.Task.Node)node).cost) 
		            return 1; 
		        return 0; 
		...
		*/
		...
		private int[] getResult() {
			...
			LinkedList<Integer> [] shortestPathReconstruction = shortestPathReconstruction(p, d);
			printShortestPathReconstruction(shortestPathReconstruction);
			return d;
		}
		...
		// BONUS
		// reconstituim drumul de lungime minima source la celelalte noduri din graf
		public LinkedList<Integer> [] shortestPathReconstruction(int[] parent, int[] d) {
			@SuppressWarnings("unchecked")
			LinkedList<Integer> shortestPathReconstruction[] = new LinkedList[n+1];
			for(int i = 1; i <= n; i++) {
				// daca avem drum de la sursa la nod 
				if(d[i] != -1) {
					shortestPathReconstruction[i] = new LinkedList<Integer>();
					int node = i;
					while(node != 0) {
						shortestPathReconstruction[i].addFirst(node);
						node = parent[node];
					}					
				}
			}
			return shortestPathReconstruction;
		}
		
		public void printShortestPathReconstruction(LinkedList<Integer> [] shortestPathReconstruction) {
			for(int i = 1; i <= n; i++) {
				// daca avem drum
				if(shortestPathReconstruction[i] != null) {
					System.out.printf("Afisam drumul minim de la sursa %d la nodul destinatie %d:\n", source, i);
					for(int k = 0; k < shortestPathReconstruction[i].size(); k++) {
						int node = shortestPathReconstruction[i].get(k);
